...
\textbf{S1--S13} (through Phase~D CIViCmine material) and figure/table labels are
prefixed \textbf{S}.
Cross-references in the main paper to ``Supplement~A'' resolve to
\S\ref{supp:prereg} (Section~S1) and so on; Phase~D material begins at
\S\ref{supp:civicmine-phase-d} (Section~S13, subsections S13.1--S13.4 as defined in
\texttt{supplement/L\_civicmine\_phase\_d.tex}).
\end{center}
...
\input{supplement/A_prereg}
\input{supplement/B_data_inventory}
\input{supplement/C_schema_full}
\input{supplement/D_trainer_spec}
\input{supplement/E_schema_wave_full}
\input{supplement/F_config_wave_full}
\input{supplement/G_coupling_slopes}
\input{supplement/H_ordinal_instability}
\input{supplement/I_lora_audit}
\input{supplement/J_additional_figures}
\input{supplement/K_stats_plan}
\input{supplement/L_civicmine_phase_d}
